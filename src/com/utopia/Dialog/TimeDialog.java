...
public class TimeDialog {
	...
	public TimeDialog(Context context, final TextView sendtime_tv,
			Button schedule_btn) {
		super();
		change = false;
		...
		timePicker.setOnTimeChangedListener(new OnTimeChangedListener() {
			...
			public void onTimeChanged(TimePicker view, int hour, int minute) {
				...
				sendtime_tv.setText(date + ", " + decimalFormat1.format(hour)
						+ ':' + decimalFormat1.format(minute));
				change = true;
			}
		...
		sure_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (change == false) {
					String date = DateUtils.getDateWN();
					date = date.substring(9, date.length() - 1);
					sendtime_tv.setText(date
							+ ", "
							+ decimalFormat1.format(timePicker.getCurrentHour())
							+ ":"
							+ decimalFormat1.format(timePicker
									.getCurrentMinute()));
				}
				closePop();
			}
	...
	public void closePop() {
		if (popupWindow != null) {
			...
			String year = sendtime_tv.getText().toString().trim()
					.substring(6, 10);
			String mounth = sendtime_tv.getText().toString().trim()
					.substring(0, 5);
			String time = sendtime_tv.getText().toString().trim()
					.substring(12, 17);
