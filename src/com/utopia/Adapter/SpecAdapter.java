...
public class SpecAdapter extends BaseAdapter {
	...
	public View getView(int paramInt, View paramView, ViewGroup paramViewGroup) {
		/*
		 * TextView localTextView; if (paramView == null) { localTextView = new
		...
		/*
		 * holder.viewBtn.setOnCheckedChangeListener(new
		 * CompoundButton.OnCheckedChangeListener() {
		 * 
		 * @Override public void onCheckedChanged(CompoundButton buttonView,
		 * boolean isChecked) { if(isChecked){ //set pre radio button
		 * if(checkedIndex != -1) { int childId = checkedIndex -
		 * listView.getFirstVisiblePosition(); if(childId >= 0){ View item =
		 * listView.getChildAt(childId); if(item != null){ RadioButton rb =
		 * (RadioButton)item.findViewById(checkedIndex); if(rb != null)
		 * rb.setChecked(false); } } } //set cur radio button checkedIndex =
		 * buttonView.getId(); } } });
		 */
		return paramView;
	}
