...
public class OrdersSalerecordAdapter extends BaseAdapter implements
	...
	public void open() {
		...
		Log.i("tag", m_CallCursor.getCount() + "aaaaaaaaaaaaa");
		Log.i("tag", "当前桌子的编号" + Constant.table_id);
	}
	...
	public int getContactId() {
		...
		if (this.m_CallCursor.moveToPosition(0)) {
			customerId = m_CallCursor.getInt(m_CallCursor
	...
	public Boolean getPriority() {
		int priority = 0;
		if (this.m_CallCursor.moveToPosition(0)) {
			priority = this.m_CallCursor.getInt(m_CallCursor
	...
	public View getView(int paramInt, View paramView, ViewGroup paramViewGroup) {
		...
		if (paramView == null) {
			...
			// localAppItem2.discount_text = (Button)
			// localView.findViewById(R.id.discount_text);
			// localAppItem2.menu_discount = ((Button)
			// localView.findViewById(R.id.menu_discount));
		...
		locald_SaleRecord.setId(this.m_CallCursor.getInt(this.m_CallCursor
				.getColumnIndex("id")));
		locald_SaleRecord.setItemNo(this.m_CallCursor
				.getString(this.m_CallCursor.getColumnIndex("itemNo")));
		...
		locald_SaleRecord.setStatus1(this.m_CallCursor
		...
		localAppItem1.menu_status.setText(locald_SaleRecord.getStatus1());
		...
		localAppItem1.notes.setText("Size:"
				+ locald_SaleRecord.getOtherSpecNo1() + " , Hotness:"
				+ locald_SaleRecord.getOtherSpecNo2() + " ,Notes:"
				+ locald_SaleRecord.getOtherSpec());
		// 获得总价
		 ...
		 * 
		 * @Override public void onClick(View arg0) { if (((CheckBox)
		 * arg0).isChecked()) { d_SaleRecord sr = (d_SaleRecord) arg0.getTag();
		 * localAppItem1.menu_check.setEnabled(false); new
		 * RefreshAsyncTask().execute(sr); }
		 * 
		 * } });
		 */
	...
	// sr[0].getPdtName(), Constant.table_id, sr[0].getId(), //
	// 原先是sr[0].getBILLID()
	// sr[0].getItemNo(), sr[0].getCreateTime());
	// sr[0].setStatus1("Doned");
	// // flag = new
	// // JsonResolveUtils(context).setSaleRecordFinish(sr[0]);//原先
	// flag = (new JsonResolveUtils(context))
	// .setSaleandpdtFinish(sr[0]);
	// } catch (Exception e) {
	// e.printStackTrace();
	// }
	// return flag;
	...
	public void onClick(View paramView) {
		...
		case R.id.menu_delete:
			if ((((d_SaleRecord) paramView.getTag()).getStatus1()
					.equals("Not Sent"))) {
		...
		case R.id.qty_add:
			if (((d_SaleRecord) paramView.getTag()).getStatus1().equals(
					"Not Sent")) {
		 ...
		 * case R.id.discount:
		 * 
		 * new pop_discount(context, paramView, ((d_SaleRecord)
		 * paramView.getTag())); break;
		 */
